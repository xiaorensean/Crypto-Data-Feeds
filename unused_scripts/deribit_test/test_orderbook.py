...
from deribit_api.deribitWS import get_last_trades_by_currency, get_orderbook, get_index, subscribe_orderbook_data
...
def subscribe_orderbook(ticker):
    try:
        #data = get_orderbook(ticker,200)
        data = subscribe_orderbook_data(ticker)
    except Exception as err:
        print (err)
        data = None
        pass
...
def get_rolling_tickers():
    option_tickers = metadata.get_options_endpoints(['ETH'],['ticker'])
    processes = {}
...
#    option_tickers = ["book."+i.split(".")[1]+".none.20.100ms"for i in option_tickers_raw]
    #for i in option_tickers:
    #    print(i)
    #    subscribe_orderbook(i)
#    processes = {} 
#    for symb in option_tickers:
#        print(symb)
#        time.sleep(0.1)
