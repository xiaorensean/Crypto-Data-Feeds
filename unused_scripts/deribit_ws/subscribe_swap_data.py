'''
Subscribes to all metadata for perpetual swaps, 
...
def create_process(client, endpoint):
  ...
  process.start()
  with open("swap_children.txt", "a+") as f:
    lines = ['----------\n', endpoint + "\n", str(process.pid) + "\n", '----------\n']
    f.writelines(lines)
  entry = {
    "endpoint": endpoint,
...
if __name__ == "__main__":
  open("swap_children.txt", 'w').close()
  client = DeribitInfluxClient()

  processes = []
  for endpoint in metadata.get_swap_endpoints():
