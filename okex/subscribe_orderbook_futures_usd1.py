...
def get_future_tickers():
    ...
    symbol_usd = []
    symbol_usdt = []
    for sr in symbol_raw:
        dt_current = int(str(datetime.datetime.now()).split(" ")[0].replace("-","")[4:])
        dt_symb = int(sr.split("-")[1].split("T")[1])
        # usd symbol
        if dt_symb < dt_current:
            symbol_usd.append(sr.split("-")[0] + "-" + sr.split("-")[1].split("T")[0] + "-" + str(int(str(datetime.datetime.now())[:2])+1) + \
                sr.split("-")[1].split("T")[1])
        else:
            symbol_usd.append(sr.split("-")[0] + "-" + sr.split("-")[1].split("T")[0] + "-" + str(datetime.datetime.now())[:2] + \
            sr.split("-")[1].split("T")[1])
        # usdt symbol
        if dt_symb < dt_current:
            symbol_usdt.append(sr.split("-")[0] + "-" + sr.split("-")[1].split("T")[0] + "T-" + str(int(str(datetime.datetime.now())[:2])+1) + \
                sr.split("-")[1].split("T")[1])
        else:
            symbol_usdt.append(sr.split("-")[0] + "-" + sr.split("-")[1].split("T")[0] + "T-" + str(datetime.datetime.now())[:2] + \
            sr.split("-")[1].split("T")[1])
...
def subscribe_orderbook_future_usd_cluster_1():
    fut_tickers = sorted(get_future_tickers()[0])
...
if __name__ == '__main__':
    subscribe_orderbook_future_usd_cluster_1()
    while True:
        time.sleep(35)
        subscribe_orderbook_future_usd_cluster_1()
