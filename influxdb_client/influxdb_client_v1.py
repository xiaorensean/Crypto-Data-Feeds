...
class InfluxClient:
    ...
    def __init__(self):        
        ...
        self.client = InfluxDBClient(host=HOST, port=8086, username=user_name, password=passwords)
...
if __name__ == '__main__':
    ...
    data = db.query_tables("okex_orderbook", ["*", "limit 1"], "raw")[0][0]
    print(data)
    #init_dt = db.query_tables("binance_orderbook_spot", ["*","where symbol='INJUSDT' limit 1"])["time"].tolist()[0]
    #end_dt = db.query_tables("binance_orderbook_spot", ["*","where symbol='INJUSDT' order by time desc limit 1"])["time"].tolist()[0]
    #init_dat = datetime.datetime.strptime(init_dt.replace("T"," ").split(".")[0],'%Y-%m-%d %H:%M:%S')
    #end_dat = datetime.datetime.strptime(end_dt.replace("T"," ").split(".")[0],'%Y-%m-%d %H:%M:%S')
    #tm_delta = end_dat - init_dat
    #dats = []
    #for t in range(tm_delta.days+1):
    #    update_dat = init_dat + datetime.timedelta(t)
    #    dats.append(str(update_dat))

    #for idx in range(len(dats)-1):
    #    print(dats[idx],dats[idx+1])
    #    data = db.query_tables("binance_orderbook_spot", ["*","where symbol='INJUSDT' and time > '{}' and time <= '{}'".format(dats[idx],dats[idx+1])])
    #    print(data)
    #s    data.to_csv(dats[idx]+".csv",index=False)
    ...
    #db.delete('FTX_trades',None,"2019-08-28 00:02:02")
    table_names = [i["name"] for i in db.get_table_name()]
    print(table_names)
    #dts = []
    #for tn in table_names:
    #    data = db.query_tables(tn,["*","limit 1"],"raw")[0][0]
    #    print(data)
    #    dt = data["time"]
    #    print(dt)
    #    dts.append(dt)
